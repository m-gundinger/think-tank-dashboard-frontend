...
- is it possible to combine the logic in "My tasks" and in the project detail page? Some things will be project-view only, but everything else should stay the same, so it might be possible to reduce redundancy and the number of files by just combining. The views and the buttons (sort, columns, new task) should be like in my tasks currently. Also, there should be a settings button for both (like currently in project details), with the possible settings depending on what makes sense for my tasks/projects.
- on the backend, there should be 4 task template categories: global, user-specific, workspace-specific, project-specific.
